...
class GLTFLoader extends _threeModule.Loader {
  constructor(manager) {
    super(manager);
  ...
  load(url, onLoad, onProgress, onError) {
    ...
    const loader = new _threeModule.FileLoader(this.manager);
  ...
  parse(data, path, onLoad, onError) {
    ...
    if (typeof data === 'string') {
      ...
      const magic = _threeModule.LoaderUtils.decodeText(new Uint8Array(data, 0, 4));
...
class GLTFLightsExtension {
  ...
  _loadLight(lightIndex) {
    ...
    let lightNode;
    const color = new _threeModule.Color(0xffffff);
...
class GLTFMeshStandardSGMaterial extends _threeModule.MeshStandardMaterial {
  constructor(params) {
    super();
...
class GLTFCubicSplineInterpolant extends _threeModule.Interpolant {
...
const _q = new _threeModule.Quaternion();
...
class GLTFParser {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (loader.isImageBitmapLoader === true) {
          onLoad = function (imageBitmap) {
            const texture = new _threeModule.Texture(imageBitmap);
            texture.needsUpdate = true;
            resolve(texture);
          };
        }
  ...
  createUniqueName(originalName) {
    const sanitizedName = _threeModule.PropertyBinding.sanitizeNodeName(originalName || '');
  ...
  loadMesh(meshIndex) {
    ...
    return Promise.all(pending).then(function (results) {
      ...
      const group = new _threeModule.Group();
  ...
  loadAnimation(animationIndex) {
    ...
    return Promise.all([Promise.all(pendingNodes), Promise.all(pendingInputAccessors), Promise.all(pendingOutputAccessors), Promise.all(pendingSamplers), Promise.all(pendingTargets)]).then(function (dependencies) {
      ...
      for (let i = 0, il = nodes.length; i < il; i++) {
        ...
        const interpolation = sampler.interpolation !== undefined ? INTERPOLATION[sampler.interpolation] : _threeModule.InterpolateLinear;
        const targetNames = [];
        ...
        for (let j = 0, jl = targetNames.length; j < jl; j++) {
          ...
          if (sampler.interpolation === 'CUBICSPLINE') {
            track.createInterpolant = function InterpolantFactoryMethodGLTFCubicSpline(result) {
              ...
              const interpolantType = this instanceof _threeModule.QuaternionKeyframeTrack ? GLTFCubicSplineQuaternionInterpolant : GLTFCubicSplineInterpolant;
  ...
  loadNode(nodeIndex) {
    ...
    return function () {
      ...
      if (nodeDef.matrix !== undefined) {
        const matrix = new _threeModule.Matrix4();
  ...
  loadScene(sceneIndex) {
    ...
    const scene = new _threeModule.Group();
      ...
      node.traverse(function (mesh) {
        ...
        for (let j = 0, jl = jointNodes.length; j < jl; j++) {
          ...
          if (jointNode) {
            ...
            const mat = new _threeModule.Matrix4();
...
function computeBounds(geometry, primitiveDef, parser) {
  ...
  const box = new _threeModule.Box3();
  ...
  if (targets !== undefined) {
    const maxDisplacement = new _threeModule.Vector3();
    const vector = new _threeModule.Vector3();
  ...
  const sphere = new _threeModule.Sphere();
